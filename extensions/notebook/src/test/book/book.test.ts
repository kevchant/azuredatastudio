		...
		while(iterations--) {
			runs.forEach(function (run) {
				describe('BookTreeViewProvider.Commands on ' + run.it, function (): void {
					...
					it('openExternalLink should open link  @UNSTABLE@', async () => {
						let executeCommandSpy = sinon.spy(vscode.commands, 'executeCommand');
						let notebookPath = run.folderPaths.markdownFile;
						bookTreeViewProvider.openMarkdown(notebookPath);
						should(executeCommandSpy.calledWith('markdown.showPreview')).be.true('openMarkdown should have called markdown.showPreview');
					...
					it('should call showPreviewFile on openBook when showPreview flag is set  @UNSTABLE@', async () => {
					...
					it('should get notebook path with untitled schema on openNotebookAsUntitled  @UNSTABLE@', async () => {
					...
					it('openNotebookFolder without folderPath should prompt for folder path and invoke loadNotebooksInFolder  @UNSTABLE@', async () => {
